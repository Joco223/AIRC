...
void ServerSocket::checkForConnections(SDL2_2D_Context& ctx, std::string pswd, std::string userName, MessageList& userList) {
	int numActiveSockets = SDLNet_CheckSockets(socketSet, 8);
	...
	if(serverSocketActivity != 0) {
		...
		if(clientCount < maxClients) {	
			...
			if(accepted) {
				...
				userList.addMessage(ctx, "", "", clientNames[freeSpot]);
				std::string empty = "";
